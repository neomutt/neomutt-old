...
void test_nntp_path2_canon(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    path.canon = (char *) mutt_str_dup(tests[i].first);
    TEST_CASE(path.orig);
    path.type = MUTT_NNTP;
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_cmp(path.canon, tests[i].second) == 0);
    }
    FREE(&path.canon);
  }
}
...
void test_nntp_path2_parent(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_cmp(parent->orig, tests[i].second) == 0);
...
void test_nntp_path2_pretty(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc > 0)
    {
      ...
      TEST_CHECK(mutt_str_cmp(path.pretty, tests[i].second) == 0);
    }
  }
}
...
void test_nntp_path2_tidy(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    path.orig = mutt_str_dup(tests[i].first);
    TEST_CASE(path.orig);
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_cmp(path.orig, tests[i].second) == 0);
    }
    FREE(&path.orig);
  }
}
