...
void test_nm_path2_canon(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_cmp(path.canon, second) == 0);
    }
    FREE(&path.canon);
  }
}
...
void test_nm_path2_parent(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    TEST_CHECK(rc == tests[i].retval);
    TEST_CHECK(mutt_str_cmp(parent ? parent->orig : NULL, second) == 0);
  }
}
...
void test_nm_path2_pretty(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc > 0)
    {
      ...
      TEST_CHECK(mutt_str_cmp(path.pretty, second) == 0);
    }
  }
}
...
void test_nm_path2_tidy(void)
{
  ...
  for (size_t i = 0; i < mutt_array_size(tests); i++)
  {
    ...
    if (rc == 0)
    {
      ...
      TEST_CHECK(mutt_str_cmp(path.orig, second) == 0);
    }
    FREE(&path.orig);
  }
}
