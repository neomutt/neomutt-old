...
int imap_path2_canon(struct Path *path, char *user, int port)
{
  ...
  mutt_str_replace(&path->canon, mutt_buffer_string(&buf));
...
int imap_path2_compare(struct Path *path1, struct Path *path2)
{
  ...
  {
    rc = mutt_str_cmp(url1->user, url2->user);
    if (rc != 0)
      goto done;
  }

  rc = mutt_istr_cmp(url1->host, url2->host);
  if (rc != 0)
    goto done;
  ...
  bool i1 = (mutt_istr_cmp(url1->path, "INBOX") == 0);
  bool i2 = (mutt_istr_cmp(url2->path, "INBOX") == 0);
  if (i1 && !i2)
  {
    rc = -1;
    goto done;
  ...
  rc = mutt_str_cmp(url1->path, url2->path);
...
int imap_path2_parent(const struct Path *path, char delim, struct Path **parent)
{
  ...
  if ((url->path[0] == '\0') || (mutt_str_cmp(url->path, "INBOX") == 0))
    goto done;
  ...
  *parent = mutt_path_new();
  (*parent)->orig = mutt_str_dup(mutt_buffer_string(buf));
...
int imap_path2_pretty(struct Path *path, const char *folder)
{
  ...
  if (url1->scheme != url2->scheme)
    goto done;
  if (mutt_istr_cmp(url1->host, url2->host) != 0)
    goto done;
...
{
  if (!mutt_istr_startswith(path->orig, "imap://") &&
      !mutt_istr_startswith(path->orig, "imaps://"))
  {
    return -1;
  }
...
int imap_path2_tidy(struct Path *path)
{
  ...
  if (!url->path || (url->path[0] == '\0') ||
      ...
      (mutt_istr_cmp(url->path, "inbox") == 0))
  {
    url->path = "INBOX";
  }
  ...
  mutt_str_replace(&path->orig, mutt_buffer_string(buf));
