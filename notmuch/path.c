...
int qsort_urlquery_cb(const void *a, const void *b)
{
  ...
  int rc = mutt_str_cmp(uq1->name, uq2->name);
  if (rc != 0)
    return rc;

  return mutt_str_cmp(uq1->value, uq2->value);
}
...
int nm_path2_pretty(struct Path *path, const char *folder)
{
  ...
  if (mutt_str_cmp(url1->path, url2->path) != 0)
    goto done;
  ...
  mutt_str_replace(&path->pretty, mutt_buffer_string(&buf));
  rc = 1;
